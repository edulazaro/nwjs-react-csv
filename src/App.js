...
class App extends React.Component {
  ...
  async fetchFile() {
    return fetch(this.filePath).then(function (response) {
      ...
      complete: (result) => {
        ...
        for (const bikeData of result.data) {
          ...
          bikes[bikeId].addStatus({
            ...
            arrivalTime: bikeData[2] ? moment(bikeData[2].replace('T', ' '), 'YYYYMMDDhh:mm:ss')/*.format('YYYY-MM-DD hh:mm:ss')*/ : false,
            departureTime: bikeData[3] ? moment(bikeData[3].replace('T', ' '), 'YYYYMMDDhh:mm:ss')/*.format('YYYY-MM-DD hh:mm:ss')*/ : false,
          });
        }
